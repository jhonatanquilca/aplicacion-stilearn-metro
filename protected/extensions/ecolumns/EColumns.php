...
//        parent::init();
        ...
        if (is_string($this->fixedLeft))
            $this->fixedLeft = array($this->fixedLeft);
        if (is_string($this->fixedRight))
            $this->fixedRight = array($this->fixedRight);
...
//                });
    ...
    protected function getWidgetColumns() {
        ...
        foreach ($this->columns as $key => $column) {
            if (in_array($key, $fixed))
                continue;
            if (array_key_exists($key, $widgetColumns))
                continue;
            $widgetColumns[$key] = array(
                'header' => $this->getColumnHeader($key, $column),
            );
    ...
    protected function getColumnHeader($key, $column) {
        if (is_string($column)) {
            ...
            $name = $matches[1];
            if (isset($matches[5]))
                return $matches[5];  //header specified in format "Name:Type:Label"
            if ($this->model instanceOf CModel)
                return $this->model->getAttributeLabel($name);
            return $name; //header = name
        } else {
            if (is_array($column)) {
                if (isset($column['header']))
                    return $column['header'];
                if (isset($column['name']) && $this->model instanceOf CModel)
                    return $this->model->getAttributeLabel($column['name']);
            }
            return $key;
        }
    }
